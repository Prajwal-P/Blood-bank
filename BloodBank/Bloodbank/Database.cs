    ...
    public class Database
    {
        ...
        public Database()
        {
            if (!File.Exists("../../BloodBankDB.sqlite3"))
            {
                ...
                cmd = new SQLiteCommand(query, con);
                cmd.ExecuteNonQuery();
                query = "CREATE TABLE DONOR (PH_NO NUMBER(10) PRIMARY KEY, DOB DATE NOT NULL, WEIGHT NUMBER(2) NOT NULL, LAST_DONATION_DATE DATE NOT NULL, FOREIGN KEY (PH_NO) REFERENCES USER(PH_NO) ON DELETE CASCADE);";
                cmd = new SQLiteCommand(query, con);
                cmd.ExecuteNonQuery(); query = "CREATE TABLE ORDERS (OR_ID INTEGER PRIMARY KEY AUTOINCREMENT, B_GRP CHAR(10) NOT NULL, RECIP_ID NUMBER(10) NOT  NULL, DONOR_ID NUMBER(10) NOT NULL, MI_ID INTEGER NOT NULL, REQ_DATE DATE NOT NULL, DEL_DATE DATE, QUANTITY INTEGER NOT NULL, FOREIGN KEY(RECIP_ID) REFERENCES USER(PH_NO) ON DELETE CASCADE, FOREIGN KEY(DONOR_ID) REFERENCES DONOR(PH_NO) ON DELETE CASCADE, FOREIGN KEY(MI_ID) REFERENCES MED_INST(MI_ID) ON DELETE CASCADE);";
                cmd = new SQLiteCommand(query, con);
                cmd.ExecuteNonQuery();
                con.Close();
            }
